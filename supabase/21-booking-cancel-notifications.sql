-- Cancel → student notify; student book → admin notify
-- Safe to run even if 20 was skipped: creates required tables first.
...
stable
as $$
  select trim(to_char(p_start at time zone 'America/Los_Angeles', 'Dy Mon FMDD, FMHH12:MI AM'))
    || ' PT';
$$;

-- Keep student self-notify helper
...
begin
  ...
  select b.slot_id, b.status, b.student_id, s.start_time
  into v_slot_id, v_status, v_student_id, v_start
  from public.bookings b
...
drop function if exists public.student_book_series_slot(uuid, integer, boolean, uuid);

-- Book one slot into an existing rolling series (+ optional admin notify)
...
-- Summary notify for multi-slot weekly booking from the client
create or replace function public.student_notify_booking_summary(
  p_weekly boolean,
  p_duration_minutes integer,
  p_pay_later boolean,
...
grant execute on function public.student_notify_booking_summary(boolean, integer, boolean, timestamptz[]) to authenticated;

-- Internal helpers — only other security definer functions should call these
