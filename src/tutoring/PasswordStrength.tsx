...
/** Tutoring-friendly rules (8+ chars so invites stay usable). */
export const tutoringPasswordRules: readonly PasswordRule[] = [
...
export function usePasswordStrength(
  ...
  const state = useMemo(() => {
    ...
    const allMet = unmet.length === 0 && value.length > 0 && !guessable
    ...
    return {
      ...
      guessable,
      announcement,
      allMet: allMet || (unmet.length === 0 && value.length > 0),
    }
  }, [value, rules, labels])
...
const TONES = {
  none: { bar: 'bg-line', text: 'text-ink-muted' },
  danger: { bar: 'bg-red-500', text: 'text-red-700' },
  caution: { bar: 'bg-amber-500', text: 'text-amber-700' },
  safe: { bar: 'bg-sage', text: 'text-sage-deep' },
} as const
    ...
    <div className={`w-full ${className}`}>
      ...
      >
        {Array.from({ length: max }, (_, i) => (
          <div key={i} className="relative h-1.5 overflow-hidden rounded-sm bg-sand">
            <motion.span
              className={`absolute inset-0 origin-left rounded-sm transition-colors duration-200 ${tone.bar}`}
              initial={false}
      ...
      <div className="mt-2 flex h-5 items-center justify-between gap-3">
        ...
        <motion.span
          aria-hidden
          className="whitespace-nowrap text-[11.5px] leading-5 text-amber-700"
          initial={false}
        ...
        <ul className="mt-3 grid gap-1.5">
          {evaluated.map((rule) => (
            <li key={rule.id} className="flex items-center gap-2">
              <span className="relative grid size-[14px] shrink-0 place-items-center rounded border border-line text-white">
                <motion.span
                  className="absolute inset-0 rounded-[3px] bg-sage"
                  initial={false}
